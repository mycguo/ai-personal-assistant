...
def transcribe_from_link(link, categories: bool):
	...
	def get_vid(_id):
		with youtube_dl.YoutubeDL(ydl_opts) as ydl:
			meta = ydl.extract_info(_id, download=True)
			downloaded_path = ydl.prepare_filename(meta)
			requested = meta.get('requested_downloads') or []
			if requested and requested[0].get('filepath'):
				downloaded_path = requested[0]['filepath']
			return meta, downloaded_path
	...
	try:
		...
		st.error(f"Unexpected YouTube processing error: {err}")
		return None

	if not os.path.exists(save_location):
		st.error("Downloaded audio file not found. Please try another YouTube link.")
		return None

	st.write('Saved audio to', save_location)
